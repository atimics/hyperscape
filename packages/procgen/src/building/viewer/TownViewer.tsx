/**
 * TownViewer
 * React component for viewing generated towns with buildings
 */
...
export const TownViewer: React.FC<TownViewerProps> = ({
  initialSeed = 12345,
  initialSize = "village",
  width = "100%",
  height = 600,
    ...
    if (showInternalRoads && town.internalRoads) {
      ...
      for (const road of town.internalRoads) {
        ...
        const roadMat = new THREE.MeshStandardMaterial({
          color: road.isMain ? 0x8b7355 : 0x9a8462, // Darker for main road
          roughness: 0.9,
        });
    ...
    for (const building of town.buildings) {
      if (showBuildings3D && BUILDING_RECIPES[building.type]) {
        ...
        if (result) {
          ...
          townGroupRef.current.add(result.mesh);
        }
      } else {
        ...
        const buildingMat = new THREE.MeshStandardMaterial({
          color: BUILDING_COLORS[building.type] ?? 0x888888,
          roughness: 0.8,
        });
